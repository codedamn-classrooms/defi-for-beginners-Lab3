...
it("should return the total supply", async () => { 
    const supply = await token.totalSupply(); //return as with an object 
    assert(supply.eq(initialBalance)); //console.log(supply); 
});
...
it("should transfer token", async () => { 
  ...
  from: accounts[0],to: accounts[1],tokens: value,
});
});
...
it("should transfer token when approved", async () => { let allowance; let receipt; const value = web3.utils.toBN(100);
